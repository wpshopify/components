...
function ItemsReducer(state, action) {
  switch (action.type) {
    ...
    case 'MERGE_QUERY_PARAMS': {
      var mergedParams = update(state.queryParams, { $merge: action.payload });

      if (!isString(mergedParams.sortKey)) {
        var finalParms = update(mergedParams, {
          sortKey: {
            $set: mergedParams.sortKey.key,
          },
        });
      } else {
        var finalParms = mergedParams;
      }

      return {
        ...state,
        queryParams: finalParms,
      };
    }
